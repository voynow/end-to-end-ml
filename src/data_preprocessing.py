...
def data_prep_pipeline(df: pd.DataFrame) -> Tuple[torch.Tensor, List[int]]:
...
def get_training_data() -> Tuple[torch.Tensor, List[int]]:
    df = load_data("training.csv")
    return data_prep_pipeline(df)


def get_validation_data() -> Tuple[torch.Tensor, List[int]]:
